...
export function Visualization({ data }: { data: string[] }) {
  // Count occurrences of each rage quit method
  const counts = data.reduce((acc, method) => {
    acc[method] = (acc[method] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  // Format data for the bar chart, including options with 0 votes
  const chartData = rageQuitOptions.map((option, index) => ({
    method: option.label,
  ...
  const chartConfig = {
    ...
    ...Object.fromEntries(
      rageQuitOptions.map((option, index) => [
        option.id,
        {
          label: option.label,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>How RC Developers Rage Quit</CardTitle>
      </CardHeader>
      <CardContent>
